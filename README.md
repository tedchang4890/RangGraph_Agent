# 맞춤형 운동 추천 멀티 에이전트 시스템 (Multi-Agent Exercise Recommender)

사용자의 현재 신체 상태와 고민(증상)을 분석하여, 이를 해결할 수 있는 **최적의 운동을 추천해 주는 AI 에이전트 파이프라인**입니다. 

LangGraph를 활용하여 여러 에이전트가 각자의 역할을 수행하며(증상 추출 ➡️ 운동 추천 ➡️ 최종 답변 정리), 로컬 LLM인 Ollama를 사용하였습니다.
...
- **Framework:** 
  ...
  - LangGraph (상태 기반 멀티 에이전트 워크플로우 구축)
...
1. **증상 추출 에이전트 (Extractor Agent)**
   - 사용자의 자연어 질문에서 핵심적인 신체 상태나 증상(예: 체력 저하, 체중 증가 등)만 정확하게 **추출**합니다.
2. **후보 추천 에이전트 (Matcher Agent)**
   - 추출된 증상 데이터를 바탕으로, 해당 문제를 개선하는 데 가장 효과적인 운동 후보 3가지를 논리적으로 **추론**합니다.
3. **답변 생성 에이전트 (Answer Agent)**
   - 앞선 에이전트들의 결과(증상, 운동 후보군)를 종합하여, 사용자가 읽기 쉽도록 개조식(Bullet points)으로 최종 **답변**을 정리해 줍니다.

---

## 그래프 구조 (LangGraph Architecture)
...
<img width="910" height="526" alt="스크린샷 2026-05-13 15 56 26" src="https://github.com/user-attachments/assets/bd44cad6-4003-414b-a0ea-d4d791e13db8" />
